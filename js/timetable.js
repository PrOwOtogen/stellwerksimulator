...
import { workingPossible, turnPossible, canReachPlatform } from './interlocking.js';
import { hhmm, parseTime } from './sim.js';
import { mulberry32 } from './events.js';

export const GATTUNGEN = [
  { code: 'ICE', vmax: 250, length: 4, weight: 1, stopChance: 0.8, dwell: 180, accel: 0.65, brake: 0.9, name: 'Intercity-Express' },
  { code: 'IC', vmax: 200, length: 5, weight: 1, stopChance: 0.9, dwell: 150, accel: 0.55, brake: 0.8, name: 'Intercity' },
  { code: 'RE', vmax: 160, length: 3, weight: 3, stopChance: 1.0, dwell: 120, accel: 0.95, brake: 1.0, name: 'Regional-Express' },
  { code: 'RB', vmax: 120, length: 2, weight: 3, stopChance: 1.0, dwell: 90, accel: 1.1, brake: 1.1, name: 'Regionalbahn' },
  { code: 'S', vmax: 120, length: 2, weight: 2, stopChance: 1.0, dwell: 60, accel: 1.25, brake: 1.2, name: 'S-Bahn' },
  { code: 'Güterzug', vmax: 100, length: 7, weight: 2, stopChance: 0.1, dwell: 300, accel: 0.25, brake: 0.5, name: 'Güterzug' },
  { code: 'Nahgüterzug', vmax: 80, length: 4, weight: 1, stopChance: 0.4, dwell: 600, accel: 0.3, brake: 0.55, name: 'Nahgüterzug' },
  { code: 'Sonderzug', vmax: 120, length: 4, weight: 0, stopChance: 0.5, dwell: 120, accel: 0.7, brake: 0.9, name: 'Sonderzug' },
  { code: 'Lok', vmax: 60, length: 1, weight: 0, stopChance: 0, dwell: 0, accel: 0.8, brake: 1.0, name: 'Lokfahrt/Rangierabteilung' }
];
export const gattungOf = code => GATTUNGEN.find(g => g.code === code) || GATTUNGEN[3];

/**
 * Befahrbare Relationen des Gleisplans.
 * Neben Durchfahrten werden auch Wendefahrten erfasst – dort endet der Zug
 * am Bahnsteig, wechselt die Fahrtrichtung und fährt zurück.
 */
export function relations(L, opts = {}) {
  ...
  for (const from of es) for (const to of es) {
    ...
    if (!a || !b) continue;
    if (from !== to && workingPossible(L, a, null, b)) {
      out.push({ from, to, kind: 'durchfahrt', viaPfs: pfs.filter(pf => workingPossible(L, a, pf, b)) });
    }
    if (opts.turns !== false) {
      const wendePfs = pfs.filter(pf => turnPossible(L, a, pf, b));
      if (wendePfs.length) out.push({ from, to, kind: 'wende', viaPfs: wendePfs });
    }
  }
  return out;
}
...
export function generateTimetable(L, count = 12, startTime = 6 * 3600, seed = 1234, opts = {}) {
  ...
  const alle = relations(L);
  // Durchfahrten mit Halt haben Vorrang; Wendefahrten nur, wenn es keine gibt (Kopfbahnhof)
  const durchMitHalt = alle.filter(r => r.kind === 'durchfahrt' && r.viaPfs.length);
  const wendeMitHalt = alle.filter(r => r.kind === 'wende' && r.viaPfs.length);
  const rels = durchMitHalt.length ? durchMitHalt
    : wendeMitHalt.length ? wendeMitHalt
      : alle.filter(r => r.kind === 'durchfahrt');
  ...
  for (let i = 0; i < count; i++) {
    ...
    const stops = [];
    if (rel.kind === 'wende' && rel.viaPfs.length) {
      // Kopfbahnhof: Einfahrt, Halt, Wende, Rückfahrt
      const pf = rel.viaPfs[Math.floor(rng() * rel.viaPfs.length)];
      const arr = t + 180 + Math.round(rng() * 120);
      stops.push({ platform: pf, arr, dep: arr + g.dwell, connections: [] });
      rows.push({
        nr, gattung: g.code, entry: rel.from, entryTime: t, exit: rel.to,
        stops, vmax: g.vmax, length: g.length, accel: g.accel, brake: g.brake,
        turn: {
          nr: g.code + ' ' + (1000 + Math.floor(rng() * 8999)), gattung: g.code,
          exit: rel.to, dep: arr + g.dwell + 300, wende: 240, stops: []
        }
      });
      continue;
    }
    ...
    const row = {
      nr, gattung: g.code, entry: rel.from, entryTime: t, exit: rel.to,
      stops, vmax: g.vmax, length: g.length, accel: g.accel, brake: g.brake, turn: null
    };
...
export function checkTimetable(L) {
  ...
  for (const row of L.timetable) {
    ...
    const wende = !!row.turn;
    if (!wende && !workingPossible(L, a, null, b))
    ...
    for (const st of row.stops) {
      ...
      if (wende) {
        if (!canReachPlatform(L, a, st.platform))
          msgs.push(`⚠ ${row.nr}: Bahnsteig ${st.platform} ist von ${row.entry} aus nicht erreichbar.`);
      } else if (!workingPossible(L, a, st.platform, b))
    ...
    if (row.turn) {
      ...
      if (!es.includes(row.turn.exit)) msgs.push(`⚠ ${row.nr}: Wende-Ziel „${row.turn.exit}" gibt es nicht.`);
        ...
        const zurück = entryByName(L, row.turn.exit)?.cell;
        const letzter = row.stops[row.stops.length - 1];
        if (zurück && letzter && !canReachPlatform(L, zurück, letzter.platform))
          msgs.push(`⚠ ${row.nr}: von ${letzter.platform} aus ist ${row.turn.exit} nicht erreichbar.`);
      }
    }
  }
...
export function renderTimetable(tbody, L, onChange, hooks = {}) {
  ...
  L.timetable.forEach((row, i) => {
    ...
    cell(sel(row.gattung, GATTUNGEN.map(g => g.code), v => {
      row.gattung = v;
      const g = gattungOf(v);
      row.vmax = g.vmax; row.length = g.length; row.accel = g.accel; row.brake = g.brake;
    }));
